...
class RunIrsFetchXmlTests(TestCase):
    ...
    def test_ein_search_with_fetch_xml_populates_parsed(self, mock_connector):
        ...
        # parsed must expose concrete typed attributes so the new
        # FinancialSnapshot.objects.create() path does not crash.
        # Fields used by the job code: tax_year (int), return_type (str),
        # taxpayer_name (str or None), financials.* (int or None),
        # governance.* (int/bool or None), total_reportable_comp_from_org
        # (int or None), num_employees (int or None), parse_quality (float),
        # schedule_l_transactions (list), schedule_r_orgs / schedule_o_explanations.
        parsed = mock.Mock()
        parsed.tax_year = 2024
        parsed.return_type = "990"
        parsed.taxpayer_name = "Test Org"
        parsed.num_employees = None
        parsed.total_reportable_comp_from_org = None
        parsed.parse_quality = 0.9
        parsed.schedule_l_transactions = []
        parsed.schedule_r_orgs = None
        parsed.schedule_o_explanations = None
        fin_mock = mock.Mock()
        fin_mock.total_contributions = None
        fin_mock.program_service_revenue = None
        fin_mock.investment_income = None
        fin_mock.other_revenue = None
        fin_mock.total_revenue = None
        fin_mock.grants_paid = None
        fin_mock.salaries_and_compensation = None
        fin_mock.professional_fundraising = None
        fin_mock.other_expenses = None
        fin_mock.total_expenses = None
        fin_mock.revenue_less_expenses = None
        fin_mock.total_assets_boy = None
        fin_mock.total_assets_eoy = None
        fin_mock.total_liabilities_boy = None
        fin_mock.total_liabilities_eoy = None
        fin_mock.net_assets_boy = None
        fin_mock.net_assets_eoy = None
        parsed.financials = fin_mock
        gov_mock = mock.Mock()
        gov_mock.voting_members_governing_body = None
        gov_mock.independent_voting_members = None
        gov_mock.schedule_l_required = None
        gov_mock.conflict_of_interest_policy = None
        gov_mock.whistleblower_policy = None
        gov_mock.document_retention_policy = None
        parsed.governance = gov_mock

        mock_connector.parse_990_xml.return_value = parsed
