...
class FindingIntakeSerializerTests(SimpleTestCase):
    ...
    def test_valid_minimal(self):
        # severity is now required (intake always validates it against VALID_SEVERITIES)
        s = self._make_serializer(
            {"title": "Test Finding", "narrative": "Some narrative.", "severity": "HIGH"}
        )
    ...
    def test_narrative_is_optional(self):
        # narrative is not required by FindingIntakeSerializer — it defaults to ""
        # A payload with title + severity (both required) and no narrative is valid
        s = self._make_serializer({"title": "Test", "severity": "HIGH"})
        self.assertTrue(s.is_valid())
    ...
    def test_invalid_confidence(self):
        # confidence was removed from FindingIntakeSerializer.allowed_fields;
        # submitting it produces an unexpected-field error under non_field_errors
        s = self._make_serializer(
            {
                "title": "Test",
                "narrative": "Text",
    ...
    def test_invalid_status(self):
        # status is not in FindingIntakeSerializer.allowed_fields (use FindingUpdateSerializer
        # to change status); submitting it produces an unexpected-field error under non_field_errors
        s = self._make_serializer(
            {
                "title": "Test",
                "narrative": "Text",
    ...
    def test_unexpected_fields(self):
        # unknown fields are rejected; error key is non_field_errors (not __all__)
        s = self._make_serializer(
            {
                "title": "Test",
                "narrative": "Text",
    ...
    def test_legal_refs_must_be_list(self):
        # legal_refs must be a list; a plain string is rejected under legal_refs key
        # payload also includes required severity so legal_refs validation is reached
        s = self._make_serializer(
            {
                "title": "Test",
                "narrative": "Text",
    ...
    def test_valid_legal_refs(self):
        # legal_refs accepts a list of strings; payload includes required severity
        s = self._make_serializer(
            {
                "title": "Test",
                "narrative": "Text",
    ...
    def test_invalid_detection_id(self):
        # detection_id was removed from allowed_fields; any value is an unexpected
        # field rejected under non_field_errors (not detection_id)
        s = self._make_serializer(
            {
                "title": "Test",
                "narrative": "Text",
    ...
    def test_detection_id_rejected(self):
        # detection_id was removed from allowed_fields; even a valid UUID is rejected
        # (the field no longer exists — it's an unexpected-field error)
        s = self._make_serializer(
            {
                "title": "Test",
                "narrative": "Text",
...
class FindingUpdateSerializerTests(SimpleTestCase):
    ...
    def _make_serializer(self, data):
        ...
        # is_valid() reads instance.status for the default and instance.investigator_note
        # for the DISMISSED guard; configure them so those reads don't produce Mock objects
        mock_instance.status = "NEW"
        mock_instance.investigator_note = ""
    ...
    def test_empty_title_accepted(self):
        # FindingUpdateSerializer does NOT validate emptiness on title/narrative —
        # it stores whatever the caller sends (blanking a field is a user choice).
        # A payload of {"title": ""} is a valid partial update.
        s = self._make_serializer({"title": ""})
        self.assertTrue(s.is_valid())

    def test_empty_narrative_accepted(self):
        # Same as above: whitespace-only narrative passes; the serializer does not
        # enforce non-empty on update (blanking is intentional edit, not an error).
    ...
    def test_unexpected_fields(self):
        # unexpected fields produce an error under non_field_errors (not __all__)
