...
_RULES: dict[str, list[tuple[re.Pattern, int]]] = {
    ...
    "PARCEL_RECORD": [
        ...
        # Ohio auditor parcel card patterns
    ...
    "OCC_REPORT": [
        (re.compile(r"\boffice of the comptroller of the currency\b", re.I), 10),
        (re.compile(r"\b\bocc\b.{0,40}\bexamination\b", re.I), 9),
        (re.compile(r"\bcomptroller of the currency\b", re.I), 8),
        (re.compile(r"\bnational bank examiner\b", re.I), 8),
        (re.compile(r"\bsafety and soundness\b", re.I), 6),
        (re.compile(r"\bbank supervision\b", re.I), 5),
        (re.compile(r"\bmatters requiring attention\b", re.I), 7),
    ],
    "CIC_REPORT": [
        (re.compile(r"\bcommunity interest company\b", re.I), 10),
        (re.compile(r"\bcic\s+report\b", re.I), 10),
        (re.compile(r"\bcic\s+regulator\b", re.I), 9),
        (re.compile(r"\bcommunity interest\s+(?:company|report)\b", re.I), 8),
        (re.compile(r"\basset lock\b", re.I), 6),
        (re.compile(r"\bdividend cap\b", re.I), 6),
        (re.compile(r"\bcommunity benefit\b", re.I), 4),
    ],
    "REFERRAL_MEMO": [
