...
def _is_plausible_person_name(name: str) -> bool:
    ...
    # A single real token paired with a stopword (e.g. "Date Sarah") is junk.
    if len(non_stop) < 2:
        return False
...
# OCR artifacts, or page headers (e.g. "PM Bright Future Foundation Inc")
...
def extract_entities(text: str, doc_type: str = "OTHER") -> dict[str, list[dict[str, Any]]]:
    ...
    # the org regex swallows it: "SARAH MITCHELL Mitchell Development Group, LLC"
    ...
    for m in _ORG_PATTERN.finditer(text):
        ...
        # Strip leading timestamp junk ("PM Bright Future..." → "Bright Future...")
        raw = _ORG_LEADING_JUNK.sub("", raw)
        # Strip leading person name ("SARAH MITCHELL Bright Future..." → "Bright Future...")
