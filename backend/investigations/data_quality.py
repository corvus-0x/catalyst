    ...
    if tax_year is None:
        ...
        result.add_issue(
            ValidationIssue(
                ...
                severity="ERROR",
                message=(
                    f"tax_year {tax_year} is outside valid range (1990–{date.today().year + 1})."
                ),
                raw_value=tax_year,
            )
        )
        ...
        if value < min_val:
            result.add_issue(
                ValidationIssue(
                    ...
                    severity="WARNING",
                    message=(
                        f"{description} ({value:,}) is below minimum ({min_val:,}). "
        ...
        if value > max_val:
            result.add_issue(
                ValidationIssue(
                    ...
                    severity="WARNING",
                    message=(
                        f"{description} ({value:,}) exceeds maximum ({max_val:,}). "
                        f"Possible OCR error or extra digit."
                    ),
                    raw_value=value,
                )
            )
