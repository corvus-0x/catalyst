...
def _generate_forensic_filename(
    ...
    Examples:
        2022-09-15_BrightFuture-Ministries-Inc_Parcel-Record.pdf
        2018-01-23_BrightFuture-Ministries_SOS-Articles-of-Inc.pdf
        2017-09-01_Westbrook-LLC_Warranty-Deed.pdf
...
def api_case_export(request, pk):
    ...
    referrals = list(case.referral_targets.order_by("-created_at"))
    ...
    writer.writerow(["ID", "Agency", "Status", "Reference Number", "Created At", "Notes"])
    for r in referrals:
        writer.writerow(
            [
                str(r.pk),
            ...
            if parsed.officers and org:
                ...
                for officer in parsed.officers:
                    if not officer.name:
                        continue
                    try:
                        result = resolve_person(officer.name, case, document=_xml_doc)
                        ...
                        logger.warning(
                            ...
                            extra={"officer_name": officer.name, "case_id": str(case.pk)},
                        )
...
def api_research_ohio_sos(request, pk):
    ...
    POST body (JSON):
        {
            "query": "Bright Future Ministries",             # required: entity name
    ...
    POST body (JSON):
        {
            ...
            "name": "MITCHELL"                            # optional: name to search for
        }
...
def api_research_add_to_case(request, pk):
    ...
    # Recorder source doesn't use the data payload — check it before the generic data guard.
    if source == "recorder":
        return JsonResponse(
            {
                "error": (
