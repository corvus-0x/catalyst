...
**Last updated:** 2026-06-21 (Session 50 — **Case Map Phase 1B + Phase 2 shipped** (PRs #14, #15) and deploying to prod. The Investigate tab is now the **Case Map workspace**: a persistent Cytoscape map reading `/case-map/` with abstract markers (shape = type, color = state) and strength-weighted edges, plus a fixed right **inspector** driven by a two-tier focus reducer (*selection is inspector state; frame is history*) — Subject / Relationship / Thread inspectors + a "What's Missing" readiness rail. `navStack`/`onAngleActive`/`ConnectionDetailPanel` removed. PR #15 also shipped **4 CI gates** (banned-strings, missing-migration, gitleaks secret-scan, AuditLog append-only — code-only, no migration). 88 Vitest + tsc clean; Playwright-verified live on the PR-15 preview. Next: Phase 3 (Thread Path Mode) / Phase 4 (Thread Builder) or the 1A fast-follow collectors.)
