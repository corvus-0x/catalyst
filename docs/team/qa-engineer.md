...
**AI endpoints:**
- `/api/cases/{id}/ai/ask/` — **async** (returns 202 + job_id, not 200 + answer). Poll `GET /api/jobs/<id>/` until SUCCESS. Frontend `aiAsk()` client handles polling transparently.
- `/api/cases/{id}/ai/analyze-patterns/` — async (202 + job_id). Writes AI-sourced Findings.
- ~~`/api/ai/summarize/`~~ ~~`/api/ai/connections/`~~ ~~`/api/ai/narrative/`~~ — **deleted Session 43.** These endpoints no longer exist.

AI endpoint test considerations:
- Expect 202, not 200 — a 200 from `/ai/ask/` means a routing bug
- Poll `GET /api/jobs/<id>/` every 2s; expect SUCCESS within 60s under normal load
- May fail gracefully (FAILED status) if Claude API is down — check `ANTHROPIC_API_KEY` is set in Railway env vars
