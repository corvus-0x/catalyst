...
describe("useAsyncJob", () => {
    ...
    it("transitions idle → queued → running → success", async () => {
        ...
        await act(async () => {
            await result.current.run({ query: "bright future" });
        });
...
3. Run an IRS name search for "bright future". Expected: spinner with "Searching… (typically 20–90 seconds)"; eventually results render.
...
def seeded_case(db):
    ...
    person = Person.objects.create(name="Sarah Example")
...
def test_build_context_includes_case_and_entities(seeded_case):
    ...
    assert any(p["name"] == "Sarah Example" for p in ctx["entities"]["persons"])
