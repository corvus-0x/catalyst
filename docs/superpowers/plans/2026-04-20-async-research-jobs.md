...
class RunCountyParcelSearchTests(TestCase):
    def setUp(self):
        ...
        self.job = SearchJob.objects.create(
            ...
            query_params={"query": "Smith", "county": "Franklin"},
        )
...
class ConvertedResearchViewTests(TestCase):
    ...
    def test_research_parcels_returns_202(self, mock_async):
        resp = self.client.post(
            ...
            data=json.dumps({"query": "Smith", "county": "Franklin"}),
            content_type="application/json",
        )
        self.assertEqual(resp.status_code, 202)
        job = SearchJob.objects.first()
...
Open a case's Research tab at http://localhost:5173/cases/<some-case-id>/research, pick "IRS 990 E-File Search", type `bright future`, click Search.
