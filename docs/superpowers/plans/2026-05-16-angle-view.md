...
**Note on `context_note`:** The entity_links `context_note` field contains a label like "Karen Homan is the buyer" or just the entity name. If it's a full sentence, trim it. To get just the entity name, look at `finding.entity_links` — the entity name comes from the context_note. If context_note is too verbose (e.g. "Karen Homan is the buyer"), show just the first few words before "is" — but actually `context_note` on entity_links is usually just the entity name or a short role label (see api-contract.md Section 6). Use it as-is.
