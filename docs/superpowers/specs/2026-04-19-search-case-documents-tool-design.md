...
```json
{
  ...
  "tool_calls_made": [
    {"name": "search_case_documents", "input": {"query": "Caldwell"}, "match_count": 3}
  ],
...
3. POST to `/api/cases/<bright-future-uuid>/ai/ask/` with questions designed to trigger the tool:
   - *"Does any document mention payments to Sarah's construction company?"* — should fire `search_case_documents` with a query like "Sarah" or "construction".
