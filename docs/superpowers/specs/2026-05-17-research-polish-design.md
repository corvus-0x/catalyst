...
**Problem:** Ohio SOS results can name persons as statutory agents, organizers, or officers. When that person is deceased (e.g. the Ronald J. Homan scenario in Do Good), the investigator needs a visual warning before acting on the result.
...
```json
[
  { "full_name": "Ronald J. Homan", "date_of_death": "2025-09-30" }
]
```
...
**Name matching rule:** Normalize both sides to lowercase+trimmed. Use `includes()` substring check (catches "Ronald J. Homan" matching inside a longer string like "Agent: Ronald J. Homan, Cassella OH"). False positive risk is low given the small deceased-person set per case.
