...
code, not just ship it. He does NOT want to debug alone — Codex handles that. He DOES want to
...
- **GREEN:** Codex acts autonomously (code style, file organization, refactoring)
- **YELLOW:** Codex recommends + Tyler confirms (new libraries, architecture changes, external API choices)
- **RED:** Codex presents options + Tyler decides (scope changes, data source priorities, UX direction)
...
| **Lead** | AI pattern analysis result (`FindingSource.AI`) | NEVER show "Sonnet", "Codex", "AI", "LLM" — call it "Lead". |
| **Intake** | Document extraction pipeline | NEVER show "Haiku", "Codex", "AI" — call it "Intake". |
...
**Banned strings in any user-visible text:** "Haiku", "Sonnet", "Opus", "Codex", "AI assistant", "LLM", "GPT"
