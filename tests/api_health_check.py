...
def check(
    ...
    accepted = expected_statuses if expected_statuses is not None else [
        expected_status]
    ...
    error = ""
    if not passed:
        expected_label = "/".join(str(s) for s in accepted)
        error = f"Expected {expected_label}, got {status}"
...
# Cross-case signal list was removed in the Finding consolidation.
...
if CASE_ID:
    ...
    check("Case dashboard",
          "/api/cases/{id}/dashboard/", "GET", status, body, dur)

    # Case documents
    status, body, dur = api_get(f"/api/cases/{CASE_ID}/documents/")
    check("Case documents",
          "/api/cases/{id}/documents/", "GET", status, body, dur)
    ...
    check("Case financials",
          "/api/cases/{id}/financials/", "GET", status, body, dur)

    # Case findings
    status, body, dur = api_get(f"/api/cases/{CASE_ID}/findings/")
    check("Case findings",
          "/api/cases/{id}/findings/", "GET", status, body, dur)

    # Save a finding ID if available
    FINDING_ID = None
    finding_list = (
        body if isinstance(body, list) else (
            body.get("results", []) if isinstance(body, dict) else [])
    )
    if finding_list and len(finding_list) > 0:
        FINDING_ID = finding_list[0].get("id")

    # Case referral targets (replaced old /referrals/ endpoint)
    status, body, dur = api_get(f"/api/cases/{CASE_ID}/referral-targets/")
    check("Case referral targets",
          "/api/cases/{id}/referral-targets/", "GET", status, body, dur)
    ...
    status, body, dur = api_post(
        ...
        {
            "title": "CSRF Test Finding",
            "severity": "MEDIUM",
            "narrative": "Seeded from health check.",
        },
    )
    check(
    ...
    status, body, dur = api_post(
        f"/api/cases/{CASE_ID}/reevaluate-findings/", {})
    check(
        "POST case re-evaluate findings (CSRF)",
        "/api/cases/{id}/reevaluate-findings/",
        "POST",
        status,
        body,
        dur,
    ...
    # This endpoint expects multipart files. JSON should return 400.
    status, body, dur = api_post(
        f"/api/cases/{CASE_ID}/documents/bulk/", {"documents": []})
    check(
        "POST document bulk upload (CSRF)",
        "/api/cases/{id}/documents/bulk/",
        "POST",
        status,
        body,
        dur,
    ...
    status, body, dur = api_post(
        f"/api/cases/{CASE_ID}/documents/process-pending/", {})
    check(
...
check("404 on missing case",
      "/api/cases/{fake}/", "GET", status, body, dur, expected_status=404)
...
print(
    f"  SECTION 4a (POST CSRF): {csrf_passed}/{csrf_tests} CSRF tests passed")
...
if durations:
    ...
    print(
        f"  PERFORMANCE: avg={avg:.0f}ms, max={max(durations):.0f}ms, slow(>2s)={len(slow)}")
